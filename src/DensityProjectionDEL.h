#ifndef DENSITYPROJECTIONDEL_H
#define DENSITYPROJECTIONDEL_H
...
class DensityProjectionDEL : public DisplayElement{
...
#define BUFSIZE 512
    ...
    FortranLinalg::DenseVector<TPrecision> density;
    float bandwidth;
    bool invert;
  ...
  public:
    ...
    DensityProjectionDEL(Data<TPrecision> &d, Animator &a, Font &f) 
      ...
      using namespace FortranLinalg;
        ...
        //compute density
        bandwidth = 0.1;
        density = DenseVector<TPrecision>(data.centered.N());
        knn = DenseMatrix<int>(200, data.centered.N());
        knnD = DenseMatrix<TPrecision>(200, data.centered.N());
        Geometry<TPrecision>::computeANN(data.centered, knn, knnD, 10);
        computeDensity();
        invert = false;
      };
    ...
    void display(void){
      ...
      for(int i=0; i<data.P.N(); i++){
        ...
        data.color(i, col);
        if(invert){
          col[3] = (1-density(i))*alpha;
        }
        else{
          col[3] = density(i)*alpha;
        }
        glColor4fv(col);
        glVertex2f(x1, x2);
  ...
  private:
    ...
    void computeDensity(){
      ...
      TPrecision maxW = 0; 
      for(int i=0; i < data.centered.N(); i++){
        TPrecision sumw = 0;
        for(int j=0; j < knnD.M(); j++){
          sumw += exp( -( knnD(j, i) * knnD(j, i) ) / (bandwidth*bandwidth) );
        }
        if(sumw >maxW){
          maxW = sumw;
        }
      ...
      FortranLinalg::Linalg<TPrecision>::Scale(density, 1.0/maxW, density);
    };
  ...
  public:
    ...
    // grab a density value for some index
    TPrecision getDensityFromIndex(int index){
